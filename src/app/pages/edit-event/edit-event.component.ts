...
export class EditEventComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  saveChanges(): void {
    ...
    // If the user did NOT pick a new Google location (coordinates are 0,0),
    // preserve the original event coordinates to avoid overwriting with zeros.
    const formCoords = formValue.coordinates;
    const hasNewCoords = formCoords && (formCoords.lat !== 0 || formCoords.lng !== 0);
    const finalCoordinates = hasNewCoords ? formCoords : this.event.coordinates;

    const updatedEvent = { ...this.event, ...formValue, coordinates: finalCoordinates };
